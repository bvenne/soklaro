...
import { WiCloudy, WiDayCloudy, WiDaySunny, WiDaySunnyOvercast, WiFog, WiNa, WiNightClear, WiRain, WiShowers, WiSleet, WiSnow, WiSprinkle, WiStormShowers, WiThunderstorm } from 'react-icons/wi';
...
export default function WeatherApp() {
  ...
  const locate = async () => {
    const result = await requestLocation(); setLocationStatus(result.status);
    if (result.status !== 'allowed') return;
    const coordinates = roundCoordinates(result.coordinates, precision);
    choosePlace({ id: `geo:${coordinates.latitude},${coordinates.longitude}`, name: 'Aktueller Standort', ...coordinates });
  };
    ...
    <main className="weather-app"><PwaRegister />
      ...
      <section className="forecast-content">
        ...
        <div className="hourly" tabIndex={0} aria-label="Horizontale 48-Stunden-Prognose">
          {forecast.hourly.map((hour, index) => <article key={`${hour.time}-${index}`} className={index === 0 ? 'now' : ''}><time>{index === 0 ? 'Jetzt' : hour.time.slice(11, 16)}</time><span className="weather-glyph" role="img" aria-label={interpretWmo(hour.weatherCode).label}>{hour.isDay ? <WiDaySunny aria-hidden="true" /> : <WiNightClear aria-hidden="true" />}</span><strong>{Math.round(hour.temperature)}°</strong><small>{hour.precipitationProbability}%</small></article>)}
        </div>
